...
pw <- read_dir("data/npo/public_works", "xlsx", filename = T, skip = 1,
               col_types = "text")
...
npo_type = syms(c("公益法人の場合", "公益法人の区分"))
...
pw <- pw %>%
  mutate(
    ...
    grantee_clean = str_remove(grantee_clean, ".*法人"),
...
check <- pw %>% filter(is.na(grantee)) %>% select(grantee_clean, grantee, grantee_detail)
...
pw <- pw %>%
  mutate(
    ...
    contract_amount_est = as.numeric(contract_amount)
  )
...
# Export to CSV
write_csv(pw, "data/npo/public_works_clean.csv")
